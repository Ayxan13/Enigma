...
#include "EnigmaTest.hpp"
#include <random>
#include <iterator>

EnigmaCpp::EnigmaTest::TestFailedException::TestFailedException(std::string_view const msg)
	: std::runtime_error(std::string{ msg })
{
}

void EnigmaCpp::EnigmaTest::EncryptTest()
{
...
void EnigmaCpp::EnigmaTest::Assert(bool condition, std::string_view const msg)
{
	if (!condition)
		throw TestFailedException();
}
void EnigmaCpp::EnigmaTest::DecryptTest()
{
	using namespace RotorTypeSpecific;
...
void EnigmaCpp::EnigmaTest::PlugBoardTest()
{
	using namespace RotorTypeSpecific;
	...
	{ // 6 plugs
		...
		auto const input = "WRBHFRROSFHBCHVBENQFAGNYCGCRSTQYAJNROJAKVKXAHGUZHZVKWUTDGMBMSCYQSKABUGRVMIUOWAPKCMHYCRTSDEYTNJLVWNQY"sv;
		auto const expectedOutput = "FYTIDQIBHDONUPAUVPNKILDHDJGCWFVMJUFNJSFYZTSPITBURMCJEEAMZAZIJMZAVFCTYTKYORHYDDSXHBLQWPJBMSSWIPSWLENZ"sv;
		auto const output = e.Encrypt(input);
		Assert(expectedOutput == output);
	}
	...
	{ // 10 plugs
		...
		auto const input = "RNXYAZUYTFNQFMBOLNYNYBUYPMWJUQSBYRHPOIRKQSIKBKEKEAJUNNVGUQDODVFQZHASHMQIHSQXICTSJNAUVZYIHVBBARPJADRH"sv;
		auto const expectedOutput = "CFBJTPYXROYGGVTGBUTEBURBXNUZGGRALBNXIQHVBFWPLZQSCEZWTAWCKKPRSWOGNYXLCOTQAWDRRKBCADTKZGPWSTNYIJGLVIUQ"sv;
		auto const output = e.Encrypt(input);
		Assert(expectedOutput == output);
	}
}
